...
import { tex, type SpineName } from '../assets'
...
// Owns the order-bubble Container and the per-slot reward state machine.
// Lives in the scene's top-most bubblesLayer (parented by ClientQueue), so
// each client has their bubble attached separately from their character
// container. Client composes one of these; update() drives the per-slot
// reward animation independently of the character's walk state.
export class ClientOrderBubble extends Container {
  ...
  constructor(private spineName: SpineName) {
    super()
    this.eventMode = 'none'
    this.visible = false
  ...
  // Phase 'check': drop a checkmark into the slot at alpha 0 — update()
